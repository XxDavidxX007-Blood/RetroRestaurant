...
Al terminar, actualiza el contador de resultados:
```js
document.getElementById('infoResultados').innerText = 'Mostrando ' + visibles + ' productos filtrados';
```

Cuando no hay filtros activos (al cargar la página o al limpiar), el texto lo pinta PHP directamente con `count($inventario)` — dice `"Mostrando X productos"` sin el "filtrados".
